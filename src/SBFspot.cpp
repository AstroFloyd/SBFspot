  ...
  for (int m = 0; m < cfg.archEventMonths; m++)
    {
      ...
      else if (rc != E_OK) cerr << "ArchiveEventData(user) returned an error: " << rc << endl;
      ...
      if (cfg.userGroup == UG_INSTALLER)
	{
	  ...
	  if (rc == E_EOF) break; // No more data (first event reached)
	  else if (rc != E_OK) cerr << "ArchiveEventData(installer) returned an error: " << rc << endl;
	}
